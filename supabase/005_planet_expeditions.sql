...
begin
 ...
 ('exterior','airlock','cockpit','corridor','crew','galley','engine','navigation','system','mine','ice','wreck') then incoming->>'scene' else 'exterior' end);
 for field,cap in select * from (values ('completed',4),('crewCompleted',4),('galleyCompleted',4),('engineCompleted',5),('airlockCompleted',3),('navigationCompleted',4),('exteriorCompleted',4),('mineCompleted',6),('iceCompleted',6),('wreckCompleted',6)) as limits(k,v)
 loop
 ...
 if not coalesce(result->'launchDone'='true'::jsonb,false) then
  ...
  if result->>'scene' in ('system','mine','ice','wreck') then result := result || '{"scene":"exterior"}'::jsonb; end if;
 end if;
...
begin
 ...
 'mineCompleted',0,'iceCompleted',0,'wreckCompleted',0,'scannerInstalled',false,'finaleDone',false,'launchDone',false,'readIds','[]'::jsonb);
