import {LanguageProvider,useLanguage} from './i18n/Language.jsx';
import {currentShipStage,chapterEntry,canVisitChapter,shipStages} from './chapterFlow.js';
...
function SceneArt({inside,airlock,completed,airlockCompleted}) {
 ...
 return <div className="art" aria-hidden="true">{t(scenes.map((src,i)=><img key={src} className="scene-image scene-layer" src={src} alt={t("")} style={{opacity:i===index?1:0}}/>))}<div className="shade"/></div>;
}
...
function App() {
 const {t}=useLanguage();
  ...
  const stage=currentShipStage(progress),nextEntry=chapterEntry(progress);
  const guided=!launchDone;
  const stageActive=launchDone||stage.scene===scene;
  const continueChapter=()=>goTo(nextEntry);
  const continueLabel=`Continue to ${nextEntry==='corridor'?'corridor':stage.name.toLowerCase()}`;
  ...
  function goTo(next) {
    ...
    if(!exploring&&!['system','mine','ice','wreck','haven','system2'].includes(next)&&!canVisitChapter(progress,next))return;
  ...
  return <main className="shell">
    <header className="masthead"><a className="brand" href="#" onClick={e => {e.preventDefault(); if(guided){continueChapter();return;} goTo(exploring?(scene==='system2'?'haven':scene!=='system'?'system':'cockpit'):navigation?'cockpit':airlockWork?'airlock':crew || galley || engine ? 'corridor' : inside || corridor ? 'airlock' : 'exterior');}}><span className="brand-mark">{t("✦")}</span>{t(" TO THE STARS")}</a><AccountButton onRestart={saveStatus.resetGame} onImport={()=>setProgress(current=>mergeProgress(current,{...initialProgress(),resetRevision:current.resetRevision}))}/></header>
    <div className="play-area"><nav className="scene-tools" aria-label={t("Scene controls")}>{t(launchDone&&<button aria-label={t("Go back")} disabled={transition} onClick={()=>goTo(exploring?(scene==='system2'?'haven':scene!=='system'?'system':'cockpit'):navigation?'cockpit':airlockWork?'airlock':crew||galley||engine?'corridor':inside||corridor?'airlock':airlock?'exterior':'airlock')}>{t("←")}</button>)}<button aria-label={t("Open ship map")} onClick={()=>setMapOpen(true)}>{t("▦")}</button>{t(launchDone&&<button aria-label={t("Open system chart")} onClick={()=>goTo('system')}>{t("✦")}</button>)}<button aria-label={t("Open ship log")} onClick={e=>openLog(e)}>{t("▤")}</button></nav><div className="scene-lives"><LivesBar/></div>
    {t(exploring?<Exploration key={scene} progress={progress} onProgress={setProgress} onNavigate={goTo} onLog={id=>setLogView({id})}/>:crew || galley || engine || airlock || airlockWork || navigation || exterior ? <CrewQuarters key={scene} room={airlock||airlockWork?'airlock':scene} repairsUnlocked={stageActive} guided={guided} nextScene={nextEntry} continueLabel={continueLabel} onContinue={continueChapter} canEnterCockpit={launchDone||(airlockCompleted===3&&['cockpit','navigation'].includes(stage.scene))} canEnterCorridor={launchDone||['crew','galley','engine'].includes(stage.scene)} canEnterShip={launchDone||stage.scene==='airlock'} launchControl={exterior&&allSystemsReady?<button className="hotspot launch-control" style={{left:'52%',top:'60%'}} aria-label={t(finaleDone?'Launch':'Start final launch challenge')} onClick={()=>finaleDone?setLaunching(true):setPlaying(finaleRepair)}><span className="target">{t("✦")}</span><span className="hotspot-label">{t(finaleDone?'Launch →':'Launch check · HARD')}</span></button>:null} engineCompleted={engineCompleted} onNavigate={goTo} completed={exterior?exteriorCompleted:navigation?navigationCompleted:airlock||airlockWork?airlockCompleted:engine?engineCompleted:galley?galleyCompleted:crewCompleted} onCompleted={exterior?setExteriorCompleted:navigation?setNavigationCompleted:airlock||airlockWork?setAirlockCompleted:engine?setEngineCompleted:galley?setGalleyCompleted:setCrewCompleted} onBack={()=>guided?continueChapter():goTo(exploring?(scene==='system2'?'haven':scene!=='system'?'system':'cockpit'):navigation?'cockpit':airlockWork?'airlock':'corridor')} exteriorView={<ExteriorArt cockpit={completed} completed={exteriorCompleted} seals={airlockCompleted===3} antenna={navigationCompleted>0}/>} onLog={id=>setLogView({id})}/> : <>
    <section ref={sceneRef} className={`game ${bubble != null ? 'story-open' : ''} ${corridor ? 'corridor' : airlock ? 'airlock' : inside ? 'inside' : 'outside'} ${celebration ? 'show-repair' : ''} ${transition ? 'travel' : ''}`} aria-label={t("Chapter one: the damaged ship")} aria-busy={transition}>
      {t(corridor ? <CorridorArt completedRoomIds={completedRoomIds}/> : <SceneArt inside={inside} airlock={airlock} completed={completed} airlockCompleted={airlockCompleted} navigationCompleted={navigationCompleted}/>)}
      {t(completed >= 2 && !airlock && !corridor && <div className={`ridge-lights ${inside ? 'ridge-inside' : 'ridge-outside'}`} aria-hidden="true"><i/><i/><i/><i/><i/></div>)}

      <div className="scene-top"><div><span className="eyebrow">{t(corridor ? 'CHAPTER 01 / INNER DECK' : airlock ? 'CHAPTER 01 / SHIP ACCESS' : 'CHAPTER 01 / STAGE 01 — COCKPIT')}</span><h1>{t(corridor ? allRoomsDone ? 'Light through the ship.' : crewCompleted===4 || galleyCompleted===4 || engineCompleted===5 ? 'A little more light.' : 'Quiet for now.' : airlock ? 'Welcome aboard.' : inside ? finished ? 'A cockpit reborn.' : 'One system at a time.' : 'A new beginning.')}</h1><p>{t(corridor ? (guided?`Next: ${stage.name}. Follow the marked door.`:'Each restored room brings a little light.') : airlock ? 'One doorway. A whole ship to restore.' : inside ? finished ? 'Follow the marked system to continue.' : 'Small repairs. A little closer to the stars.' : 'Your journey to the stars starts here.')}</p></div><div className="location"><span className="location-icon">{t("◎")}</span>{t(corridor ? 'CORRIDOR' : airlock ? 'AIRLOCK' : inside ? 'COCKPIT' : 'LANDING SITE')}</div></div>
      {t(loaded && !transition && corridor && (launchDone||stage.scene==='crew') && <button className="hotspot crew-door" style={{left:'18%',top:'37%'}} aria-label={t("Enter crew quarters")} onClick={()=>goTo('crew')}><span className="target">{t("↗")}</span><span className="hotspot-label">{t("Crew quarters")}</span></button>)}
      {t(loaded && !transition && corridor && (launchDone||stage.scene==='galley') && <button className="hotspot crew-door" style={{left:'82%',top:'37%'}} aria-label={t("Enter galley")} onClick={()=>goTo('galley')}><span className="target">{t("↗")}</span><span className="hotspot-label">{t("Galley")}</span></button>)}
      {t(loaded && !transition && corridor && (launchDone||stage.scene==='engine') && <button className="hotspot crew-door engine-door" style={{left:'50%',top:'37%'}} aria-label={t("Enter engine room")} onClick={()=>goTo('engine')}><span className="target">{t("↗")}</span><span className="hotspot-label">{t("Engine room")}</span></button>)}
      {t(loaded && !transition && scene === 'exterior' && <button className="hotspot hatch" aria-label={t("Enter the ship")} onClick={() => goTo('airlock')}><span className="target">{t("↗")}</span><span className="hotspot-label">{t("Enter the ship")}</span></button>)}
      {t(loaded && !transition && airlock && <button className="hotspot" style={{left:'29%',top:'38%'}} aria-label={t("Enter cockpit")} onClick={()=>goTo('cockpit')}><span className="target">{t("↗")}</span><span className="hotspot-label">{t("Cockpit")}</span></button>)}
      {t(loaded && !transition && airlock && <button className="hotspot airlock-corridor-door" style={{left:'83%',top:'37%'}} aria-label={t("Enter corridor")} onClick={()=>goTo('corridor')}><span className="target">{t("↗")}</span><span className="hotspot-label">{t("Corridor")}</span></button>)}
      {t(loaded && !transition && !celebration && inside && current && stageActive && <button ref={mainAction} className="hotspot repair-hotspot" style={{left:`${current.x}%`,top:`${current.y}%`}} aria-label={t(`Inspect ${current.name}`)} onClick={openRepair}><span className="target">{t(current.icon)}</span><span className="hotspot-label">{t(current.name)}</span></button>)}
      {t(inside&&finished&&loaded&&!transition&&(launchDone||stage.scene==='navigation')&&<button className={`cockpit-navigation-console ${progress.scannerInstalled?'scanner-upgraded':''}`} aria-label={t("Open navigation console")} onClick={()=>goTo('navigation')}><svg viewBox="0 0 140 80" aria-hidden="true"><path className="nav-orbit" d="M18 51Q47 3 115 28Q130 49 70 67Q32 75 18 51Z"/><path d="M27 53L57 34L90 45L117 22" fill="none" stroke="currentColor" strokeWidth="2"/><circle cx="27" cy="53" r="4"/><circle cx="57" cy="34" r="3"/><circle cx="90" cy="45" r="3"/><path d="M117 13L120 20L127 22L120 25L117 32L114 25L107 22L114 20Z"/></svg><strong>{t(progress.scannerInstalled?'LONG-RANGE SCANNER ↗':'NAVIGATION ↗')}</strong><small>{t(navigationCompleted===4?'COURSE VERIFIED':`TAP TO CONNECT · ${navigationCompleted}/4`)}</small></button>)}
      {t(celebration && inside && <div className="repair-reveal" role="region" aria-label={t("Repair completed")}>
        <span className="eyebrow">{t("REPAIR COMPLETE")}</span>
        <h2>{t(`✓ ${celebration.name}`)}</h2>
        <p role="status">{t(celebration.result)}</p>
<button className="new-log" onClick={e => openLog(e, logEntries.find(entry => entry.repair === celebration.id)?.id)}><span>{t("▤")}</span> {t(readIds.includes(logEntries.find(entry => entry.repair === celebration.id)?.id) ? 'Read ship log entry' : 'New log entry')} <span>{t("→")}</span></button>
    ...
    <dialog ref={dialog} aria-labelledby="repair-title" onClose={() => opener.current?.focus()} onClick={e => {if(e.target === dialog.current) closeRepair();}}><div className="repair-panel"><button className="close" aria-label={t("Close")} onClick={closeRepair}>{t("×")}</button><span className="eyebrow">{t("CHAPTER 01 / COCKPIT")}</span>
      {t(showList ? <><h2 id="repair-title">{t("Four steps to life.")}</h2><p>{t("Repair these systems in order. This is just the first stage of restoring the whole ship.")}</p><ol className="repair-list">{t(repairs.map((repair, i) => <li key={repair.id}><button onClick={e => openRepair(e, i)}><span className={`repair-badge ${repairState(i,completed)}`}>{t(i < completed ? '✓' : i + 1)}</span><span><strong>{t(repair.name)}</strong><small>{t(repairState(i,completed) === 'complete' ? 'Restored · view or replay' : i === completed ? 'Ready to repair' : `Requires ${repairs[i - 1].name.toLowerCase()}`)}</small></span><span>{t("→")}</span></button></li>))}</ol><div className="demo-note">{t("After the cockpit: hull, living quarters, supplies, navigation, fuel and engines. Departure comes at the end of the ship chapter.")}</div></> : <>
        <div className="repair-symbol">{t(detail.icon)}</div><h2 id="repair-title">{t(detail.name)}</h2><p>{t(detailState === 'complete' ? detail.result : detail.description)}</p>
        <div className="demo-note"><strong>{t(detailState === 'locked' ? 'Not available yet' : detailState === 'complete' ? 'System restored' : detail.lesson)}</strong><br/>{t(detailState === 'locked' ? `First restore ${repairs[inspected - 1].name.toLowerCase()}.` : detail.objective)}<br/>{t("4 in a line creates Pulse; 5 creates Nova. Valid swaps use one move.")}</div>
        {t(detailState !== 'locked' && <button className="primary" onClick={() => {closeRepair(); if (!inside) setScene('cockpit'); setPlaying(detail);}}>{t(detailState === 'complete' ? 'Replay lesson' : 'Play')} <span>{t("→")}</span></button>)}
        <button className="all-repairs" onClick={() => setShowList(true)}>{t("View all cockpit repairs")}</button>
      </>)}
    </div></dialog>
    ...
    {t(logView && <ShipLog entries={archive} completed={completed} readIds={readIds} initialId={logView.id} onRead={id => setReadIds(ids => ids.includes(id) ? ids : [...ids, id])} onClose={closeLog}/>)}
