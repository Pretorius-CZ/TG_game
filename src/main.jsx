...
function App() {
  ...
  const archive = [...logEntries.map(e=>({...e,available:e.unlockAt<=completed})),...crewLogs.map(e=>({...e,available:e.unlockAt<=crewCompleted,unlockAt:e.unlockAt+4})),...galleyLogs.map(e=>({...e,available:e.unlockAt<=galleyCompleted,unlockAt:e.unlockAt+8})),...engineLogs.map(e=>({...e,available:e.unlockAt<=engineCompleted,unlockAt:e.unlockAt+12}))];
  ...
  function goTo(next) {
    if (busy.current || next === scene || (['crew','galley','engine'].includes(next) && completed<repairs.length)) return;
  ...
  return <main className="shell">
    <header className="masthead"><a className="brand" href="#" onClick={e => {e.preventDefault(); goTo(crew || galley || engine ? 'corridor' : inside || corridor ? 'airlock' : 'exterior');}}><span className="brand-mark">✦</span> TO THE STARS</a><span className="prototype">PLAYABLE PROTOTYPE <i/></span></header>
    {crew || galley || engine ? <CrewQuarters key={scene} room={scene} completed={engine?engineCompleted:galley?galleyCompleted:crewCompleted} onCompleted={engine?setEngineCompleted:galley?setGalleyCompleted:setCrewCompleted} onBack={()=>goTo('corridor')} onLog={id=>setLogView({id})}/> : <>
...
<button className="new-log" onClick={e => openLog(e, logEntries.find(entry => entry.repair === celebration.id)?.id)}><span>▤</span> {readIds.includes(logEntries.find(entry => entry.repair === celebration.id)?.id) ? 'Read ship log entry' : 'New log entry'} <span>→</span></button>
    ...
    <button className="ship-log-button" onClick={e => openLog(e)} disabled={transition || !loaded}><span>▤ Ship log</span><span>{archive.filter(entry => entry.available && !readIds.includes(entry.id)).length ? `${archive.filter(entry => entry.available && !readIds.includes(entry.id)).length} unread` : 'Open archive'} →</span></button>
    <AudioControls/>
