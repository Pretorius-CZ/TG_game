...
<button className="new-log" onClick={e => openLog(e, logEntries.find(entry => entry.repair === celebration.id)?.id)}><span>▤</span> {readIds.includes(logEntries.find(entry => entry.repair === celebration.id)?.id) ? 'Read ship log entry' : 'New log entry'} <span>→</span></button>
    ...
    {inside&&finished&&<section className="preflight" aria-label="Ship readiness"><span className="eyebrow">DEPARTURE CHECKLIST</span><h2>{allSystemsReady?'All systems restored':'Prepare for the journey'}</h2><button className="primary" onClick={()=>goTo('navigation')}>{navigationCompleted===4?'Visit navigation':'Restore communications & navigation'} →</button><div className="readiness-list">{readiness.map(r=><button key={r.id} data-system={r.id} data-ready={r.ready} onClick={()=>goTo(r.id)}><span>{r.ready?'✓':'○'} {r.name}</span><b>{r.completed}/{r.total}</b></button>)}</div>{allSystemsReady&&<p>Course verified. The departure sequence is coming next.</p>}</section>}
