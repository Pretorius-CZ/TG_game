...
export default function CrewQuarters({completed,onCompleted,onBack,onLog,room='crew',exteriorView=null}) {
  ...
  return <>
    ...
    <nav className="scene-nav" aria-label={`${label} navigation`}><button onClick={onBack}>← {navigation?'Cockpit':airlock?'Airlock':'Corridor'}</button><span className="nav-location">{label}</span></nav>
    {completed>0&&<details className="crew-replays"><summary>Completed repairs · replay lessons</summary>{tasks.slice(0,completed).map((r,i)=><button key={r.id} onClick={()=>{setOutside(false);setReveal(null);setBubble(i);}}>{r.name} ↻</button>)}</details>}
